...
tch_var <- full_tch_2 %>% 
  ...
  mutate(tchfemale = recode(tchfemale, '2' = 1, '1' = 0),
         ...
         tchexp_cat = case_when(tchexp <= 2 ~ "0",
                                tchexp >= 3 & tchexp <= 6 ~ "1",
                                tchexp >= 7 & tchexp <= 9 ~ "2",
                                tchexp >= 10 & tchexp <= 12 ~ "3",
                                tchexp >= 13 & tchexp <= 15 ~ "4",
                                tchexp >= 16 & tchexp <= 24 ~ "5",
                                tchexp >= 25 ~ "6"))
...
descript <- function(data){
  data %>%
    ...
    mutate(tchexp_cat = factor(tchexp_cat, 
                               ...
                               labels = c("<= 2 years of experience (omitted)",
...
### Visualize the relationship between experience and score
...
rbind(chn, eng, mat) %>% 
  ...
  geom_smooth(method = "loess", color = "brown", fill = "red") +
  facet_wrap(~ subject) +
  labs(x = "Teacher experience (years)",
       y = "Student score",
       caption = "Figure 3 Panel A. Relationship between teacher experience and student score") +
  theme_bw() + 
  theme(plot.caption = element_text(hjust = 0),
        axis.text.x = element_text(size=9, angle=45)) +
...
rbind(chn, eng, mat) %>% 
  ...
  labs(x = "Teacher experience (years)",
       ...
       caption = "Figure 3 Panel B. Relationship between teacher experience and student confidence") +
  theme_bw() + 
  theme(plot.caption = element_text(hjust = 0),
        axis.text.x = element_text(size=9, angle=45)) +
  scale_x_discrete(breaks = c(5, 10, 15, 20, 25, 30, 35))
...
stargazer(m1, m2, m3, m4, m5, m6, m7, m8, m9,
          type = "html",
          keep = c("\\btchexp_cat1\\b", "\\btchexp_cat2\\b", "\\btchexp_cat3\\b", "\\btchexp_cat4\\b", "\\btchexp_cat5\\b", "\\btchexp_cat6\\b"),
          model.numbers = FALSE,
          dep.var.caption = "",
          dep.var.labels = c("", ""),
          column.labels = c("Chinese", "Chinese", "Chinese", "English", "English", "English", "Math", "Math", "Math"),
...
```{r, include = TRUE, results = "asis"}
m1 <- mgcv::gam(score ~ s(tchexp, bs='ps') + poly(chn1, 3) + poly(eng1, 3) + poly(mat1, 3) + female + age + only + rural + migrant + medu + fedu + income + hrsize + hrfemale + hrage + hronly + hrrural + hrmigrant + hrmedu + hrfedu + hrincome + advisor + tchfemale, data = chn, method="REML")
plot(m1)
p <- predict(m1, type="lpmatrix")
beta <- coef(m1)[grepl("tchexp", names(coef(m1)))]
s <- p[,grepl("tchexp", colnames(p))] %*% beta
ggplot(data=cbind.data.frame(s, chn$tchexp), aes(x=chn$tchexp, y=s)) + geom_line()


m2 <- mgcv::gam(score ~ s(tchexp, bs='ps') + poly(chn1, 3) + poly(eng1, 3) + poly(mat1, 3) + female + age + only + rural + migrant + medu + fedu + income + hrsize + hrfemale + hrage + hronly + hrrural + hrmigrant + hrmedu + hrfedu + hrincome + advisor + tchfemale, data = eng, method="REML")
plot(m2)
p <- predict(m2, type="lpmatrix")
beta <- coef(m2)[grepl("tchexp", names(coef(m2)))]
s <- p[,grepl("tchexp", colnames(p))] %*% beta
ggplot(data=cbind.data.frame(s, eng$tchexp), aes(x=eng$tchexp, y=s)) + geom_line()


m3 <- mgcv::gam(score ~ s(tchexp, bs='ps') + poly(chn1, 3) + poly(eng1, 3) + poly(mat1, 3) + female + age + only + rural + migrant + medu + fedu + income + hrsize + hrfemale + hrage + hronly + hrrural + hrmigrant + hrmedu + hrfedu + hrincome + advisor + tchfemale, data = mat, method="REML")
plot(m3)
p <- predict(m3, type="lpmatrix")
beta <- coef(m3)[grepl("tchexp", names(coef(m3)))]
s <- p[,grepl("tchexp", colnames(p))] %*% beta
ggplot(data=cbind.data.frame(s, mat$tchexp), aes(x=mat$tchexp, y=s)) + geom_line()
```
