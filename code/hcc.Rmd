...
### Teacher human capital characteristics of interest 
 ...
 - Teaching experience
   ...
   - a series of indicator variables (omitted 1 year) representing teacher experience of
     - 2-3 years (tchexp1)
     - 4-6 years (tchexp2)
     - 7-9 years (tchexp3)
     - 10-12 years (tchexp4)
...
tch_var <- full_tch_2 %>% 
  ...
  mutate(tchfemale = recode(tchfemale, '2' = 1, '1' = 0),
         ...
         tchexp_cat = case_when(tchexp <= 2 ~ "0",
                                tchexp >= 3 & tchexp <= 4 ~ "1",
                                tchexp >= 5 & tchexp <= 9 ~ "2",
                                tchexp >= 10 & tchexp <= 14 ~ "3",
                                tchexp >= 15 & tchexp <= 24 ~ "4",
                                tchexp >= 25 ~ "5"))
...
chn_full <- left_join(stu_var, tch_var_chn) %>% 
  ...
  select(ids, clsids, w2clsids, schids, fall, subject, 
         tchedu, tchgrad, tchmajor, tchexp, tchexp_cat,
...
descript <- function(data){
  data %>%
    select(tchedu, tchgrad, tchmajor, tchexp, tchexp_cat, score, conf) %>%
    mutate(tchexp_cat = factor(tchexp_cat, 
                               levels = c(0, 1, 2, 3, 4, 5),
                               labels = c("Less than one year experience (omitted)",
    ...
    tbl_summary(
      type = list(c("tchedu", "tchexp", "score", "conf") ~ "continuous",
                  c("tchgrad", "tchmajor") ~ "dichotomous",
...
stargazer(m1, m2, m3, m4, m5, m6, m7, m8, m9,
          type = "html",
          keep = c("\\btchexp_cat1\\b", "\\btchexp_cat2\\b", "\\btchexp_cat3\\b", "\\btchexp_cat4\\b", "\\btchexp_cat5\\b"),
          model.numbers = FALSE,
          dep.var.caption = "",
          dep.var.labels = c("", ""),
          column.labels = c("Chinese", "Chinese", "Chinese", "English", "English", "English", "Math", "Math", "Math"),
