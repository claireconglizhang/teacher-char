...
tch_var <- full_tch_2 %>% 
  ...
  mutate(tchfemale = recode(tchfemale, '2' = 1, '1' = 0),
         ...
         tchexp1 = ifelse(tchexp <= 1, 1, 0),
         tchexp2 = ifelse(tchexp > 1 & tchexp <= 2, 1, 0),
         tchexp3 = ifelse(tchexp > 2 & tchexp <= 3, 1, 0),
         tchexp4 = ifelse(tchexp > 3 & tchexp <= 6, 1, 0),
         tchexp5 = ifelse(tchexp > 7 & tchexp <= 9, 1, 0),
         tchexp6 = ifelse(tchexp > 9, 1, 0)) 
...
chn_full <- left_join(stu_var, tch_var_chn) %>% 
  ungroup()  %>% 
  select(ids, clsids, w2clsids, schids, fall, tchedu, tchgrad, tchmajor, tchexp, tchexp1:tchexp6,
...
descript <- function(data){
  data %>%
    select(tchedu, tchgrad, tchmajor, tchexp, tchexp1:tchexp6, score, conf) %>%
    tbl_summary(
      type = list(c("tchedu", "tchexp", "score", "conf") ~ "continuous",
                  c("tchgrad", "tchmajor", "tchexp1", "tchexp2", "tchexp3", "tchexp4", "tchexp5", "tchexp6") ~ "dichotomous"),
      ...
      label = list(tchedu ~ "Education (in years)", 
                   ...
                   tchexp1 ~ "1-2 years of experience",
                   tchexp2 ~ "3-4 years of experience",
                   tchexp3 ~ "5-6 years of experience",
                   tchexp4 ~ "7-12 years of experience",
                   tchexp5 ~ "13-24 years of experience",
                   tchexp6 ~ "25+ years of experience", 
                   score ~ "Score", 
                   conf ~ "Confidence")
      )
}
...
```{r, include = TRUE}
chn %>% 
  mutate(tchedu = as.factor(tchedu)) %>% 
  ggplot(aes(tchedu)) +
  geom_bar(aes(y = (..count..)/sum(..count..)), width = 0.5) +
  labs(x = "Teacher education attainment (years)",
       y = "Proportion of students",
       caption = "Fig.1. Distribution of student observations in Chinese sample, by teacher education attainment") +
  theme_bw() + 
...
eng %>% 
  ...
  labs(x = "Teacher education attainment (years)",
       y = "Proportion of students",
       caption = "Fig.2. Distribution of student observations in English sample, by teacher education attainment") + 
  theme_bw() + 
  theme(plot.caption = element_text(hjust = 0))
...
mat %>% 
  ...
  labs(x = "Teacher education attainment (years)",
       y = "Proportion of students",
       caption = "Fig.3. Distribution of student observations in math sample, by teacher education attainment") + 
  theme_bw() + 
  theme(plot.caption = element_text(hjust = 0))
```
...
```{r, include = TRUE}
chn %>% 
  mutate(tchexp = as.factor(tchexp)) %>% 
  ggplot(aes(tchexp)) +
  geom_bar(aes(y = (..count..)/sum(..count..))) +
  labs(x = "Teacher experience (years)",
       y = "Proportion of students",
       caption = "Fig.4. Distribution of student observations in Chinese sample, by teacher experience") + 
  theme_bw() + 
  theme(plot.caption = element_text(hjust = 0))
...
eng %>% 
  ...
  labs(x = "Teacher experience (years)",
       y = "Proportion of students",
       caption = "Fig.5. Distribution of student observations in English sample, by teacher experience") + 
  theme_bw() + 
  theme(plot.caption = element_text(hjust = 0))
...
mat %>% 
  ...
  labs(x = "Teacher experience (years)",
       y = "Proportion of students",
       caption = "Fig.6. Distribution of student observations in math sample, by teacher experience") + 
  theme_bw() + 
  theme(plot.caption = element_text(hjust = 0))
```
