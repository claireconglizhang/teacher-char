...
Naive estimates of female teacher effects:

```{r}
ols1 <- function(df, var){
  felm(var ~ tchfemale + age + only + rural + migrant + medu + fedu + income + poly(chn1, 3) + poly(eng1, 3) + poly(mat1, 3) + poly(cog1, 3) | schids | 0 | schids,  df)
}

ols2 <- function(df, var){
  felm(var ~ tchfemale + age + only + rural + migrant + medu + fedu + income + poly(chn1, 3) + poly(eng1, 3) + poly(mat1, 3) + poly(cog1, 3) + hrsize + hrfemale + hrage + hronly + hrrural + hrmigrant + hrmedu + hrfedu + hrincome | schids | 0 | schids,  df)
}

ols3 <- function(df, var){
  felm(var ~ tchfemale + age + only + rural + migrant + medu + fedu + income + poly(chn1, 3) + poly(eng1, 3) + poly(mat1, 3) + poly(cog1, 3) + hrsize + hrfemale + hrage + hronly + hrrural + hrmigrant + hrmedu + hrfedu + hrincome + tchage + tchexp + tchedu + tchadvisor | schids | 0 | schids,  df)
}
```
...
stargazer(m1, m2, m3, m4, m5, m6, m7, m8, m9,
          ...
          keep = c("\\btchfemale\\b"),
          keep.stat = c("n", "rsq"),
          covariate.labels = "Female teacher",
          model.numbers = FALSE,
          dep.var.caption = "Wave 2 Score",
          dep.var.labels = c("", "", ""),
          column.labels = c("Chinese", "Chinese", "Chinese", "English", "English", "English", "Math", "Math", "Math"),
...
stargazer(m1, m2, m13, m4, m5, m16, m7, m8, m19,
          ...
          keep = c("\\btchfemale\\b"),
          keep.stat = c("n", "rsq"),
          covariate.labels = "Femal teacher",
          model.numbers = FALSE,
...
list("Chinese score" = m13,
  ...
  labs(color = "",
       x = "Point estimates and 95% CIs",
       caption = "Figure 4. The effects of female teacher for female students")+ 
  theme(plot.caption = element_text(hjust = 0),
        legend.position = "top")
...
list("Chinese score" = m13,
  ...
  labs(color = "",
       x = "Point estimates and 95% CIs",
       caption = "Figure 5. The effects of female teacher for male students")+ 
  theme(plot.caption = element_text(hjust = 0),
        legend.position = "top")
```
